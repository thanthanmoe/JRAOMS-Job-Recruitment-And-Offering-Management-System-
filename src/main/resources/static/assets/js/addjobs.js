...
$(document).ready(function() {
	...
	if (myValue === 'success') {
		iziToast.success({ timeout: 3000, icon: 'bi bi-check', title: 'Success', message: 'Job Post successfully created!' });
	...
	$("[type=reset]").click(function() {
		var fieldIds = ['floatingName', 'numberOfPost', 'workingDays', 'workFrom', 'workTo', 'salary', 'description', 'responsibility', 'requirement', 'preference'];
		...
		for (var i = 0; i < fieldIds.length; i++) {
			...
			if (fieldId === 'responsibility') {
				$('#respo').removeClass("border border-danger");
				$('#responsibility').summernote('code', '');
			}

			if (fieldId === 'requirement') {
				$('#requ').removeClass("border border-danger");
				$('#requirement').summernote('code', '');
			}

			if (fieldId === 'preference') {
				$('#prefer').removeClass("border border-danger");
				$('#preference').summernote('code', '');
			}
		}
	...
	departmentSelect.html('<option value="" disabled selected>Select your Department</option>');
